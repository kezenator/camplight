...
int gaysign_main(int argc, char *argv[])
{
    ...
    if ((argc != 3)
        ...
        || !bbox::rt::net::TcpEndpoint::FromString(argv[2], fadecandy_remote_endpoint)
    ...
    {
        std::cerr << "Usage:  " << argv[0] << " ipv4-listen-addr:port fadecandy-server-addr:port" << std::endl;
		std::cerr << "   e.g. " << argv[0] << " 127.0.0.1:8080 127.0.0.1:7890" << std::endl;
        return 1;
    }
    ...
    ledsigns::ApplicationService app("app", proactor, http_local_endpoint, fadecandy_remote_endpoint);
