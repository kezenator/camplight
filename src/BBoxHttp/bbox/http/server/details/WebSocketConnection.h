...
public:
    ...
    WebSocketConnection(ServerWebSocket *owner, Request &request, const std::string &protocol, StateHandler &&state_handler, ReceiveHandler &&rx_handler);
    ~WebSocketConnection();

    static bool IsUpgrade(Request &request);

    void UpdateOwner(ServerWebSocket *owner);
