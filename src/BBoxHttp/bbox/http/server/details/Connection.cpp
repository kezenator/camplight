/**
  * @file
  *
  * Implementation for the bbox::http::server::details::Connection class.
  */
...
void Connection::Send(Response::ResponsePtr &&response)
{
	...
	m_state_string = bbox::Format("Sending response #%d", m_request_count);
...
void Connection::NotifyUpgradedToWebSocket()
{
	BBOX_ASSERT(!m_request_ptr);
	BBOX_ASSERT(!m_response_ptr);
	BBOX_ASSERT(!m_closed);

	m_state_string = bbox::Format("Upgrading to WebSockets for request #%d", m_request_count);
