...
server::server(scheduler& sched, const unsigned int tcp_port)
    ...
    m_async_stop_in_progress(false), m_num_check_async_stop_complete_posted(0)
{}
...
void server::start(void)
{
    ...
    if (! m_is_listening) {
        ...
        try {
            ...
            (void)e; // skip unreferenced error if logging is disabled
...
void server::async_stop(bool wait_until_finished,
                        boost::function<void()> completion_handler)
{
    // lock mutex for thread safety
    boost::mutex::scoped_lock server_lock(m_mutex);
    ...
    if (m_is_listening) {
        ...
        m_is_listening = false;
        m_async_stop_in_progress = true;
        m_async_stop_completion_handler = std::move(completion_handler);
        ...
        m_num_check_async_stop_complete_posted++;
        get_io_service().post(boost::bind(&server::check_async_stop_complete, this));
    }
}

void server::check_async_stop_complete()
{
    boost::mutex::scoped_lock server_lock(m_mutex);

    if (m_num_check_async_stop_complete_posted > 0)
        m_num_check_async_stop_complete_posted--;

    if (m_async_stop_in_progress)
    {
        prune_connections();
        ...
        if ((m_num_check_async_stop_complete_posted == 0)
            && m_conn_pool.empty())
        {
            // OK - looks like we're all done

            m_async_stop_in_progress = false;

            boost::function<void()> handler = std::move(m_async_stop_completion_handler);
