...
public:
    ...
    /**
     * stops listening for new connections, aynchronously, with complete callback.
     *
     * @param wait_until_finished if true, waits until connections are completed, else forces all connections to close.
     */
    void async_stop(bool wait_until_finished, boost::function<void()> completion_handler);
...
private:
    ...
    /// Checks if an async stop operation is complete
    void check_async_stop_complete();
    ...
    /// set to true if async stop in progress
    bool                                    m_async_stop_in_progress;

    /// completion handler for async stop processing
    boost::function<void()>                 m_async_stop_completion_handler;

    /// the number of check async close complete handlers posted
    size_t                                  m_num_check_async_stop_complete_posted;
