...
#if !defined(WIN32) // defined(PION_HAVE_UNORDERED_MAP) && defined(PION_CMAKE_BUILD)
...
// Kerry Johnson - 31 May 2014
// This may have been required in previous versions of
// Visual Studio, but Visual Studio 2013 has better
// C++11 support and this doesn't work any more.
// Changing to use the normal tr1 code
...
//#ifdef _MSC_VER
    ...
    struct ihash_windows {

        inline ihash_windows()
        {
        }

        inline ihash_windows(const ihash_windows &)
        {
        }

        inline ihash_windows &operator =(const ihash_windows &)
        {
            return *this;
        }
        ...
        inline bool operator()(const std::string& _Key1, const std::string& _Key2) const {
            return is_iless<std::string>()(_Key1, _Key2);
        }
